...
const Contact : FC<WorkProps> = ({isDarkMode,setIsDarkMode}) => {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    formData.append("access_key", process.env.NEXT_PUBLIC_YOUR_ACCESS_KEY || "");
    console.log(process.env.NEXT_PUBLIC_YOUR_ACCESS_KEY);
  ...
  return (
    <div id='contact' className='w-full px-[12%] py-10 scroll-mt-20 bg-[url("/footer-bg-color.png)] bg-no-repeat bg-center bg-[length:90%_auto] dark:bg-none'>
        ...
        <form onSubmit={onSubmit} className='max-w-2xl mx-auto'>
            ...
            <button type="submit" className='py-3 px-8 w-max flex items-center justify-between gap-2 bg-black/80 text-white rounded-full mx-auto hover:bg-black duration-500 dark:bg-transparent dark:border-[0.5px] dark:hover:bg-darkHover'>Submit Now <Image alt="" src={assets.right_arrow_white} className='w-4'/></button>
